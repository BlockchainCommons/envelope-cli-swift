...
struct ProofConfirmCommand: ParsableCommand {
    static var configuration = CommandConfiguration(commandName: "confirm", abstract: "Confirm that an elided envelope contains a target digest using a proof.", usage: "On success, print the original envelope so it can be piped to the next operation. On failure, exit with an error condition.")
